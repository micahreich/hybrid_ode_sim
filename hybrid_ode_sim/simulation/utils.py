...
from typing import Optional, List, Tuple, Union, Any, Callable, Dict
...
class BaseModel:
    def __init__(self, y0,
        ...
        self.children_models = []  # List of models which receive input from this model, will get deleted
    ...
    def history(self, interpolator_kind=None) -> Tuple[np.ndarray, np.ndarray, interp1d]:
        ...
        if interpolator_kind is None:
            interpolator_kind = 'linear' if isinstance(self, ContinuousTimeModel) else 'previous'
        ...
        def individual(model, direction: InputDirectionType) -> None:
            if direction == InputDirectionType.FORWARD:
                if self.name in model.input_models:
                    raise ValueError(f"Model {self.name} is already registered as an input to {model.name}!")
...
class DiscreteTimeModel(BaseModel):
    ...
    def discrete_dynamics(self, t: float, y: Any) -> Any: raise NotImplementedError  # y[k+1] = f(t, y[k], ...)
...
class ContinuousTimeModel(BaseModel):
    ...
    def continuous_dynamics(self, t: float, y: np.ndarray) -> np.ndarray: raise NotImplementedError  # dy/dt = f(t, y, x)
    
    def output_validate(self, y: np.ndarray) -> np.ndarray: return y
    
    def inputs_to(self, model_B) -> None:
        model_B = ensure_iterable(model_B)
        
        if any([isinstance(c, DiscreteTimeModel) for c in model_B]):
            raise ValueError(f"{self.name} (ContinuousTimeModel) cannot have DiscreteTimeModel descendants.")
