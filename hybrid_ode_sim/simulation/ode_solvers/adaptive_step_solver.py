...
class AdaptiveStepButcherTableau:
    ...
    use_higher_order_estimate: bool = field(default=True)   # Whether to use the higher order estimate or not
...
class AdaptiveStepRK(RKIntegrator):
    def __init__(self, logging_level: LogLevel=LogLevel.DEBUG,
                 h_max: float = np.inf, max_h_factor: float = 10, min_h_factor = 0.2, h0: Optional[float] = None, 
        ...
        if self.butcher_tableau.use_higher_order_estimate and np.allclose(self.butcher_tableau.A[-1],
                                                                          self.butcher_tableau.b_higher_order,
                                                                          atol=1e-9):
        ...
        assert self.butcher_tableau.b_lower_order.size == self.butcher_tableau.b_higher_order.size

    def _compute_ys_new(self, y: np.ndarray, h: float, K: np.ndarray, y_validate: Callable) -> Tuple[np.ndarray, np.ndarray]:
        y_new_higher_order = y_validate(y + h * np.dot(self.butcher_tableau.b_higher_order, K))
        y_new_lower_order = y_validate(y + h * np.dot(self.butcher_tableau.b_lower_order, K))
    ...
    def _update_step_size(self, t: float, h: float, y: np.ndarray,
                          y_new_higher_order: np.ndarray, y_new_lower_order: np.ndarray) -> Tuple[bool, float]:
        y_new = y_new_higher_order if self.butcher_tableau.use_higher_order_estimate else y_new_lower_order
        ...
        if not step_accepted and h <= self.h_min:
            self.logger.critical(f"Step size (h={h}) is too small near t={t} with continued step rejection!")
    ...
    def _initial_step_size(self, f: Callable, f0: np.ndarray, t0: float, y0: np.ndarray) -> float:
    ...
    def _step(self, t_range: Tuple[float, float], t: float, h: float,
              y: np.ndarray, f: Callable, y_validate: Callable) -> Tuple[np.ndarray, float, float]:
        step_accepted = False
        ...
        while not step_accepted:
            ...
            y_new_higher_order, y_new_lower_order = self._compute_ys_new(y, h, K, y_validate)

            step_accepted, new_h_whole = self._update_step_size(t, h, y, y_new_higher_order, y_new_lower_order)
    ...
    def solve(self, t_range: Tuple[float, float]):
        ...
        h = self._validate_h(
            t_range,
            t,
            self._initial_step_size(self.f_combined, f0_combined, t, y0_combined) if self.h0 is None else self.h0
        )
        ...
        while t < t_f:
            ...
            y_new_combined, next_h_whole, h_used = self._step(t_range, t, h, y_combined, self.f_combined,
            ...
            h = self._validate_h(t_range, t, next_h_whole)  # Update the timestep for the next iteration after validation

        self.h0 = next_h_whole                              # Update the initial step size for the next .solve() call
...
class RK45(AdaptiveStepRK):
    butcher_tableau = AdaptiveStepButcherTableau(
        ...
        b_higher_order = np.array([35/384, 0, 500/1113, 125/192, -2187/6784, 11/84, 0]),
        b_lower_order  = np.array([5179/57600, 0, 7571/16695, 393/640, -92097/339200, 187/2100, 1/40]),
        order = 5,
        n_stages = 7,
